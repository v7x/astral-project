...
cp "$root/packaging/debian/preinst" "$pkg/DEBIAN/preinst"
...
printf '%s\n' \
  ...
  'Depends: python3 (>= 3.12), python3-cbor2, python3-cryptography, python3-pyfuse3, bubblewrap (>= 0.9.0)' \
  'Description: Astral Project root broker' \
  >"$pkg/DEBIAN/control"
# ABI-bearing dependencies come from Ubuntu packages, not this build host. The
# Astral wheel is pure Python; target Python imports its certified distro modules.
printf '%s\n' 'python3-cbor2, python3-cryptography, and python3-pyfuse3 are runtime dependencies.' \
  >"$pkg/usr/share/doc/astral-project/runtime-requirements.txt"
...
cc -std=c11 -O2 -Wall -Wextra -Werror "$root/packaging/native/aspr-host-rx.c" -o "$pkg/usr/libexec/astral-project/aspr-host-rx"
...
cp "$root/packaging/launchers/aspr-homed" "$pkg/usr/libexec/astral-project/aspr-homed"
...
chmod 0555 "$pkg/usr/libexec/astral-project/aspr-bwrap-launch" \
  ...
  "$pkg/usr/libexec/astral-project/aspr-host-rx"
