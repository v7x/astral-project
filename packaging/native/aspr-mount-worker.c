...
#define RUNTIME 74
#define STAGING "/run/astral-project/staging"
...
static void make_target(char *p,unsigned char kind) {
 char *q=p+sizeof(STAGING); for(;*q;q++) if(*q=='/') { *q=0; if(mkdir(p,0700)&&errno!=EEXIST) die("mkdir"); *q='/'; }
...
static void mount_one(int source,char *target,unsigned char access) {
 int tree=syscall(SYS_open_tree,source,"",OPEN_TREE_CLONE|OPEN_TREE_CLOEXEC|AT_EMPTY_PATH);
 if(tree<0) die("open_tree");
 if(access==1) { struct mount_attr a={.attr_set=MOUNT_ATTR_RDONLY}; if(syscall(SYS_mount_setattr,tree,"",AT_EMPTY_PATH,&a,sizeof(a))) die("mount_setattr"); }
...
static void attach_runtime(void) {
 struct stat st; char target[]=STAGING RUNTIME_TARGET;
 if(fstat(RUNTIME,&st)||!S_ISDIR(st.st_mode)) bad("runtime descriptor invalid");
 make_target(target,2); mount_one(RUNTIME,target,1);
}
static void enter_synthetic_root(void) {
 if(mkdir(STAGING "/oldroot",0700)&&errno!=EEXIST) die("mkdir oldroot");
...
static void run_fixed_sftp(void) {
 ...
 if(dup2(STREAM,STDIN_FILENO)<0||dup2(STREAM,STDOUT_FILENO)<0||dup2(STREAM,STDERR_FILENO)<0) die("dup stream");
...
int main(int argc,char **argv) {
 ...
 sync_map();
 if((fcntl(PLAN,F_GET_SEALS)&(F_SEAL_WRITE|F_SEAL_SHRINK|F_SEAL_GROW|F_SEAL_SEAL))!=(F_SEAL_WRITE|F_SEAL_SHRINK|F_SEAL_GROW|F_SEAL_SEAL)) bad("plan unsealed");
 ...
 for(i=0;i<count;i++) { uint32_t slot; unsigned char access,kind; uint16_t len; char target[sizeof(STAGING)+MAX_TARGET+1];
  ...
  verify_fd(SOURCE_BASE+slot,u64(p+o+8),u64(p+o+16),u64(p+o+24),kind);
  memcpy(target,STAGING,sizeof(STAGING)-1); memcpy(target+sizeof(STAGING)-1,p+o+ENTRY,len); target[sizeof(STAGING)-1+len]=0; make_target(target,kind); mount_one(SOURCE_BASE+slot,target,access); o+=ENTRY+len;
 }
