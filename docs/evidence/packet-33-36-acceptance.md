...
- Latest exact `uv run pytest -q`: `734 passed, 1 skipped, 8 warnings`.
...
- Latest exact `uv run coverage run -m pytest -q && uv run coverage report`:
  `734 passed, 1 skipped`; `TOTAL 9921 0 2838 0 100%`.
